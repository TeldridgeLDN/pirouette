  ...
  const fetchAnalyses = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.analyses) {
        // Check for timed out analyses
        const now = Date.now();
        const analysesWithTimeout = data.analyses.map((analysis: CompetitorAnalysis) => {
          if (
            ...
            if (elapsed > ANALYSIS_TIMEOUT) {
              return { ...analysis, status: 'timed_out' as const };
            }
          }
          return analysis;
        });
        setAnalyses(analysesWithTimeout);
      }
    } catch (err) {
...
const DIMENSION_INSIGHTS: Record<string, {
  ...
  keepTip: string;
  recommendationMatch: string; // Matches recommendation.dimension
  competitorTip: string; // What competitors typically do better
}> = {
  'Colours': {
    ...
    recommendationMatch: 'Colour & Contrast',
    competitorTip: 'uses a more focused colour palette with stronger contrast',
  },
  'Whitespace': {
    ...
    recommendationMatch: 'Whitespace',
    competitorTip: 'uses more generous spacing between sections',
  },
  'Complexity': {
    ...
    recommendationMatch: 'Visual Hierarchy',
    competitorTip: 'has a simpler page structure with fewer elements',
  },
  'Typography': {
    ...
    recommendationMatch: 'Typography',
    competitorTip: 'uses clearer font hierarchy with better size contrast',
  },
  'Layout': {
    ...
    recommendationMatch: 'Layout',
    competitorTip: 'has a more balanced and grid-aligned layout',
  },
  'CTA': {
    ...
    recommendationMatch: 'CTA Design',
    competitorTip: 'has more prominent call-to-action buttons',
  },
  'Hierarchy': {
    ...
    recommendationMatch: 'Content Hierarchy',
    competitorTip: 'has clearer visual hierarchy guiding the eye',
  },
};
...
function CompetitiveInsights({ userReport, competitors }: { userReport: CompetitorReport; competitors: CompetitorReport[] }) {
  ...
  const scrollToRecommendation = (dimension: string) => {
    const insight = DIMENSION_INSIGHTS[dimension];
    if (!insight) return;
    
    // Find recommendation cards by looking for the dimension text
    const recommendationSection = document.querySelector('section h2')?.closest('section');
    if (recommendationSection) {
      recommendationSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  };

  // Generate action plan text for export
  const generateActionPlanText = () => {
    let text = `🎯 COMPETITIVE ACTION PLAN\n`;
    text += `Generated by Pirouette\n`;
    text += `${'─'.repeat(40)}\n\n`;
    
    if (gaps.length > 0) {
      text += `📉 GAPS TO CLOSE (${gaps.length}):\n\n`;
      gaps.slice(0, 3).forEach((gap, idx) => {
        const insight = DIMENSION_INSIGHTS[gap.dimension];
        text += `${idx + 1}. ${gap.dimension} (-${gap.gap} pts vs ${gap.competitor})\n`;
        if (insight) {
          text += `   Quick fix: ${insight.quickFix}\n`;
          text += `   Effort: ${insight.effort} | Impact: ${insight.impact}\n`;
          text += `   Competitor insight: ${gap.competitor} ${insight.competitorTip}\n`;
        }
        text += `\n`;
      });
    }
    ...
    if (advantages.length > 0) {
      text += `🏆 ADVANTAGES TO MAINTAIN (${advantages.length}):\n\n`;
      advantages.slice(0, 3).forEach((adv, idx) => {
        const insight = DIMENSION_INSIGHTS[adv.dimension];
        text += `${idx + 1}. ${adv.dimension} (+${adv.lead} pts ahead)\n`;
        if (insight) {
          text += `   ${insight.keepTip}\n`;
        }
        text += `\n`;
      });
    }
    ...
    if (gaps.length > 0) {
      text += `${'─'.repeat(40)}\n`;
      text += `💡 Estimated improvement: +${estimatedImprovement} points\n`;
    }
  ...
  const copyActionPlan = async () => {
    const text = generateActionPlanText();
    try {
              ...
              {gaps.slice(0, 3).map((gap, idx) => {
                ...
                return (
                  <div key={idx} className="bg-white/60 rounded-lg p-3">
                    ...
                    {insight && (
                      <>
                        {/* P3: Competitor specifics */}
                        <p className="text-xs text-red-600 mb-2 italic">
                          💡 {gap.competitor} {insight.competitorTip}
                        </p>
                        ...
                        <div className="flex items-center justify-between text-xs">
                          ...
                          {/* P1: Link to recommendations */}
                          <button
                            onClick={() => scrollToRecommendation(gap.dimension)}
        ...
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                <h4 className="font-semibold text-slate-900">Quick Action Plan</h4>
                {/* P2: Export action plan */}
                <button
