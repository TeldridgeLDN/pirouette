...
- **Pricing**: Free tier (1/week) → Pro £29/mo → Agency £99/mo
...
├── src/
│   ├── queue/
│   │   ├── config.ts      # Redis connection, queue settings, job options
│   │   ├── worker.ts      # BullMQ worker with retry logic
│   │   └── index.ts       # Re-exports
│   ├── worker.ts          # Standalone worker entry point
│   └── server.ts          # Updated with queue integration
├── Dockerfile             # Multi-mode (server/worker/all)
...
```
src/app/
├── terms/page.tsx         # Terms of Service (UK law)
├── privacy/page.tsx       # Privacy Policy (UK GDPR)
└── ethics/page.tsx        # Crawler Ethics Policy
```
...
├── global-error.tsx       # Root error catch-all
...
└── quality-validator.ts   # 10 quality checks, scoring system

src/components/
└── QualityBanner.tsx      # Quality display component
```
...
- ⚠️ BullMQ worker (needs Redis in Railway)
- ⚠️ Railway service deployment
- ⚠️ Quality validation UI integration in report page
...
### Analysis Flow
1. User submits URL → `/api/analyze` creates job in Supabase
2. Job triggers Railway service → adds to BullMQ queue
3. Worker processes: Playwright screenshot → Analysis → Save report
4. Frontend polls `/api/jobs/[id]` → redirects to `/report/[id]` when done
