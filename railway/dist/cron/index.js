...
const refresh_patterns_1 = require("./refresh-patterns");
const retry_failed_1 = require("./retry-failed");
const cleanup_stale_1 = require("./cleanup-stale");
// ============================================================================
...
function initCronJobs() {
    ...
    // Pattern refresh job
    const patternJob = new cron_1.CronJob(SCHEDULES.REFRESH_PATTERNS, async () => {
        console.log('[CRON] Starting pattern refresh...');
        const startTime = Date.now();
        try {
            const result = await (0, refresh_patterns_1.refreshPatterns)();
    ...
    // Retry failed jobs
    const retryJob = new cron_1.CronJob(SCHEDULES.RETRY_FAILED, async () => {
        console.log('[CRON] Checking for failed jobs to retry...');
        const startTime = Date.now();
        try {
            const result = await (0, retry_failed_1.retryFailedJobs)();
            const duration = Date.now() - startTime;
    ...
    jobs.push(retryJob);
    // Cleanup stale jobs
    const cleanupJob = new cron_1.CronJob(SCHEDULES.CLEANUP_STALE, async () => {
        console.log('[CRON] Cleaning up stale jobs...');
        const startTime = Date.now();
        try {
            const result = await (0, cleanup_stale_1.cleanupStaleJobs)();
            const duration = Date.now() - startTime;
...
async function runJobManually(jobName) {
    ...
    switch (jobName) {
        case 'refresh':
            return (0, refresh_patterns_1.refreshPatterns)();
        case 'retry':
            return (0, retry_failed_1.retryFailedJobs)();
        case 'cleanup':
            return (0, cleanup_stale_1.cleanupStaleJobs)();
        default:
